...
typedef struct // Struct sobre as informacoes dos navios
{
    ...
    int identificacao;
    int orientacao;
    int tamanho;//Utilizado para descobrir se o navio caiu      // NPosicoesNAtingidos
    char posicaoY;
    int posicaoX;
    int nJogada;
...
int ObtemOrientacaoDoNavio(tNavios navio);
...
tTabuleiro PosicionaNavioNosTabuleiros(tTabuleiro tabuleiro, int n){
    ...
    for(i=0;i<10;i++){
        for(j=0;j<10;j++){
            if(j==x && i==y){
                if(ObtemOrientacaoDoNavio(tabuleiro.navio[n])==1){//ORIENTACAO HORIZONTAL
                ...
                if(ObtemOrientacaoDoNavio(tabuleiro.navio[n])==0){//ORIENTACAO VERTICAL
                    tam=ObtemTamanhoDoNavio(tabuleiro.navio[n]);
                    for(a=0;a<tam;a++){
                        if(i+a>=10){
                           tabuleiro=TornaInvalidoTabuleiro(tabuleiro);
...
tJogada LeJogada(tJogador jogador){
    ...
    int cont=1;
        while(1){
            ...
            //CORRECAO: COLOQUEI UMA CONDICAO PARA O TRATAMENTO DE JOGADAS COM ALGO DIFERENTE DE UM NUMERO NA SEGUNDA POSICAO. 
            if(jog[1]>'9'|| jog[1]<'0'){
            ...
            if((jogada.posicaoY>'j'|| jogada.posicaoX>10) || jogada.posicaoX<0 || jogada.posicaoY<'a'){
                //CORRECAO COLOQUEI PARA IMPRIMIR A STRING DA JOGADA
                printf("%s:Jogada invalida! Jogue novamente %s:\n\n", jog, jogador.nome);
                cont=1;
                continue;
            }
...
void OrdenaCrescenteNavios(tNavios vet[], int qtd)
{
    ...
    //CORRECAO:Separei a parte que organiza o vetor em ordem alfabética da parte
    // que desempata, pois tinha casos que quebravam o programa ao ser realizado
    //juntos
    for (a = 0; a < qtd; a++)
    {
        auxiliar = vet[a];
